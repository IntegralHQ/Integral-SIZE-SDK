...
export abstract class TwapPair {
   ...
   */
  public static async getToken(provider: BaseProvider, chainId: ChainId, tokenAddress: string): Promise<Token> {
   ...
   */
  public static async getOracleAddress(provider: BaseProvider, chainId: ChainId, pairAddress: string): Promise<string> {
    const results = await multicall(provider, chainId, [
      {
        target: pairAddress,
  ...
  private static async getTokenDecimals(
    ...
    for (let i = 0; i < pairLength; i++) {
      ...
      if (areAddressesEqual(tokenAddress, token0Address) || areAddressesEqual(tokenAddress, token1Address)) {
