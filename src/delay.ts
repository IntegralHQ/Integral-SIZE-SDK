...
export abstract class TwapDelay {
  ...
  public static async getWithdrawCallParameters(
    ...
    const unwrap = firstValue.currency === NATIVE_CURRENCY[chainId] || secondValue.currency === NATIVE_CURRENCY[chainId]
  ...
  public static async getDepositCallParameters(
    ...
    const wrap = firstValue.currency === NATIVE_CURRENCY[chainId] || secondValue.currency === NATIVE_CURRENCY[chainId]
    const etherValue =
      ...
      calldata: this.DELAY_CODER.encodeFunctionData('deposit', [
        {
          ...
          minSwapPrice: makeFloatEncodable(
            initialPrice
              .mul(
                (options?.depositSlippageDenominator ?? SETTINGS_DENOMINATORS.depositSlippageTolerance) -
                  slippageTolerance
              )
   ...
   */
  public static async getOrderStatus(provider: BaseProvider, chainId: ChainId, orderId: number): Promise<number> {
    const results = await multicall(provider, chainId, [
      {
        target: DELAY_ADDRESS[chainId],
        callData: this.DELAY_CODER.encodeFunctionData('getOrderStatus', [orderId]),
