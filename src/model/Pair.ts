...
export class Pair {
  ...
  constructor(readonly address: string, private first: Currency, private second: Currency, private state: PairState) {
    if (first.chainId !== second.chainId) {
      throw new Error('ChainId mismatch')
    }
    this.inverted = toToken(this.second).address.toLowerCase() < toToken(this.first).address.toLowerCase()
  ...
  getLiquidity(firstValue: CurrencyValue, secondValue: CurrencyValue, proportional: boolean) {
    if (this.inverted) {
      ...
      return { swapDepositFirstAmount: swapDeposit1Amount, swapDepositSecondAmount: swapDeposit0Amount, liquidity }
    }
    const { swapDeposit0Amount, swapDeposit1Amount, liquidity } = this.getRawLiquidity(
      firstValue,
      secondValue,
    ...
    return { swapDepositFirstAmount: swapDeposit0Amount, swapDepositSecondAmount: swapDeposit1Amount, liquidity }
  }
  ...
  private getRawLiquidity(currency0: CurrencyValue, currency1: CurrencyValue, proportional: boolean) {
    ...
    return {
      ...
      swapDeposit0Amount: CurrencyValue.fromString(
        currency0.currency,
        swapDeposit0Amount.lt(0) ? '0' : formatUnits(swapDeposit0Amount, currency0.currency.decimals)
      ),
      swapDeposit1Amount: CurrencyValue.fromString(
        currency1.currency,
        swapDeposit1Amount.lt(0) ? '0' : formatUnits(swapDeposit1Amount, currency1.currency.decimals)
      ),
    }
  }
  ...
  private getRatio(amount0: BigNumber, amount1: BigNumber, reserve0: BigNumber, reserve1: BigNumber) {
  ...
  private getDepositAmount0In(amount0: BigNumber, updatedReserve0: BigNumber, updatedReserve1: BigNumber) {
    return this.oracle.depositTradeXIn(amount0, updatedReserve0, updatedReserve1)
  }

  private getDepositAmount1In(amount1: BigNumber, updatedReserve0: BigNumber, updatedReserve1: BigNumber) {
