...
describe('TwapPair', () => {
  ...
  describe('TwapPair.getToken', () => {
    it('returns tokens', async () => {
      // Arrange
      const tokenAddress = USDC[ChainId.Kovan]
      const expectedToken = new Token(ChainId.Kovan, USDC[ChainId.Kovan].toLowerCase(), 'USD Coin', 'USDC', 6)

      // Act
      const token = await TwapPair.getToken(provider, ChainId.Kovan, tokenAddress)
  ...
  describe('TwapPair.getTokenDecimals', () => {
    it('returns token0 and token1 decimals', async () => {
      ...
      // Act
      const tokenDecimals = await (<any>TwapPair).getTokenDecimals(provider, ChainId.Kovan, tokenAddress)
  ...
  describe('TwapPair.getTokenAddresses', () => {
    it('returns token0 and token1 addresses', async () => {
      ...
      // Act
      const [token0Address, token1Address] = await TwapPair.getTokenAddresses(provider, ChainId.Kovan, pairAddress)
  ...
  describe('TwapPair.getOracleAddresses', () => {
    it('returns oracle address for given pair address', async () => {
      ...
      // Act
      const oracleAddress = await TwapPair.getOracleAddress(provider, ChainId.Kovan, pairAddress)
