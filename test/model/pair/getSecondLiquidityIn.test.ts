...
describe('Pair.getSecondLiquidityIn', () => {
  it('throws for invalid parameters', () => {
    expect(() => pairAB.getSecondLiquidityIn(new CurrencyValue(TokenB, parseUnits('1')))).to.throw('Invalid parameter')
  })
  ...
  it('returns undefined for zero input', () => {
    expect(pairAB.getSecondLiquidityIn(new CurrencyValue(TokenA, parseUnits('0')))).to.deep.eq(undefined)
  })

  it('returns undefined for empty reserves', () => {
    expect(emptyPairAB.getSecondLiquidityIn(new CurrencyValue(TokenA, parseUnits('1')))).to.deep.eq(undefined)
  })
})
