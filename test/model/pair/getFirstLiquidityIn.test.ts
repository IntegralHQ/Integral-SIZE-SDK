...
describe('Pair.getFirstLiquidityIn', () => {
  it('throws for invalid parameters', () => {
    expect(() => pairAB.getFirstLiquidityIn(new CurrencyValue(TokenA, parseUnits('1')))).to.throw('Invalid parameter')
  })
  ...
  it('returns undefined for zero input', () => {
    expect(pairAB.getFirstLiquidityIn(new CurrencyValue(TokenB, parseUnits('0')))).to.deep.eq(undefined)
  })

  it('returns undefined for empty reserves', () => {
    expect(emptyPairAB.getFirstLiquidityIn(new CurrencyValue(TokenB, parseUnits('2')))).to.deep.eq(undefined)
  })
})
